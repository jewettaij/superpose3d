...
def Superpose3D(
    aaXf_original: Sequence[float],  # <-- coordinates for the "frozen" object
    aaXm_original: Sequence[float],  # <-- coordinates for the "mobile" object
    # ---- optional arguments: ----
    aWeights_original: Optional[Sequence[float]] = None,  # optional weights
    ...
    # Subtract the centers-of-mass from the coordinates of each object.
    ...
    # normalize the vector.  (It should be normalized already,
    # but just in case it is not, do it again.)
    p /= np.linalg.norm(p)
